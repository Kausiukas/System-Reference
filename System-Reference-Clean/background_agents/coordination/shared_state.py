...
class SharedState:
    ...
    async def get_recent_heartbeats(self, agent_id: str, minutes: int = 10) -> List[datetime]:
        ...
        try:
            ...
            # PostgreSQL adapter returns list of dicts with agent_id and timestamp
            heartbeat_records = await self.postgresql_adapter.get_recent_heartbeats(agent_id, minutes)
            ...
            for record in heartbeat_records:
                if isinstance(record, dict) and 'timestamp' in record:
                    ...
                    # Fallback for direct datetime objects
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get agent summary excluding placeholder slots
            agents = await self.get_registered_agents()

            # Filter out agents explicitly marked as placeholder slots
            real_agents = [
                a for a in agents
                if not (
                    a.get('state') == 'inactive' and a.get('slot_status') == 'available'
                )
            ]

            active_agents = len([a for a in real_agents if a.get('state') == 'active'])
            ...
            health_score = await self._calculate_system_health_score(
                len(real_agents), active_agents, len(error_events), db_health
            )
    ...
    # --- Feedback and Clarification Logging ---
    
    async def log_user_feedback(self, feedback: dict):
        """Log user feedback (yes/no, comment) for a help request."""
        if not hasattr(self, '_feedback_logs'):
            self._feedback_logs = []
        self._feedback_logs.append(feedback)
        # TODO: Integrate with PostgreSQL in production

    async def get_user_feedback(self, request_id: str = None, user_id: str = None):
        """Retrieve user feedback logs, optionally filtered by request_id or user_id."""
        if not hasattr(self, '_feedback_logs'):
            return []
        logs = self._feedback_logs
        if request_id:
            logs = [fb for fb in logs if fb.get('request_id') == request_id]
        if user_id:
            logs = [fb for fb in logs if fb.get('user_id') == user_id]
        return logs

    async def log_clarification_response(self, clarification: dict):
        """Log a user clarification response to a system prompt."""
        if not hasattr(self, '_clarification_logs'):
            self._clarification_logs = []
        self._clarification_logs.append(clarification)
        # TODO: Integrate with PostgreSQL in production

    async def get_clarification_responses(self, request_id: str = None, user_id: str = None):
        """Retrieve clarification responses, optionally filtered by request_id or user_id."""
        if not hasattr(self, '_clarification_logs'):
            return []
        logs = self._clarification_logs
        if request_id:
            logs = [c for c in logs if c.get('request_id') == request_id]
        if user_id:
            logs = [c for c in logs if c.get('user_id') == user_id]
        return logs

    async def get_conversation_memory(self, user_id: str):
        """Retrieve conversation memory for a user (stub: returns all help requests for user)."""
        if not hasattr(self, '_conversation_memory'):
