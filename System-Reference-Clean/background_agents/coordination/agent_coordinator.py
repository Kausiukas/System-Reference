...
from background_agents.ai_help.pynarrative_agent import PyNarrativeAgent  # Add PyNarrative import
from background_agents.monitoring.enhanced_memory_manager import EnhancedMemoryManager
...
from background_agents.monitoring.self_healing_agent import SelfHealingAgent
...
class AgentCoordinator:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info("Agent Coordinator initialization completed with memory management")
                ...
                if self.memory_manager:
                    try:
                        ...
                        if memory_result.get('success'):
                            ...
                            alerts = memory_result.get('alerts_generated', [])
                            if alerts:
                                for alert in alerts:
                                    self.logger.warning(f"Memory alert: {alert.get('message', 'Unknown alert')}")
                        else:
                            self.logger.error(f"Memory management cycle failed: {memory_result.get('error_details', 'Unknown error')}")
                    except Exception as e:
                        self.logger.error(f"Memory management monitoring error: {e}")
    ...
    async def trigger_agent_recovery(self, agent_id: str, status: AgentStatus) -> bool:
        ...
        try:
            ...
            # Persist updated recovery_attempts to prevent endless recovery loops
            try:
                await self.shared_state.update_agent_state(
                    agent_id,
                    status.state,  # keep existing state
                    {
                        'recovery_attempts': status.recovery_attempts + 1,
                        'last_recovery': datetime.now(timezone.utc).isoformat(),
                    },
                )
            except Exception as e:
                self.logger.warning(f"Could not update recovery_attempts for {agent_id}: {e}")
            ...
            await self.shared_state.log_system_event(
                ...
                {
                    'agent_id': agent_id,
                    'recovery_strategy': 'restart' if status.state == 'error' else 'refresh',
                    'success': recovery_success,
                    'previous_health_score': status.health_score,
                    'attempt_number': status.recovery_attempts + 1,
                },
    ...
    async def restart_agent(self, agent_id: str) -> bool:
        """Restart an agent in error state"""
        try:
        ...
        try:
            ...
            for agent_id in self.agents:
                try:
                    ...
                    # Skip placeholder slots (inactive and marked available)
                    if agent_data.get('state') == 'inactive' and agent_data.get('slot_status') == 'available':
                        continue

                    real_agents.append(agent_data)
            ...
            return {
                ...
                'average_health': total_health / len(real_agents) if real_agents else 0,
                'system_status': 'healthy' if active_agents >= len(real_agents) * 0.8 else 'degraded'
            }
    ...
    async def _handle_agent_startup_failure(self, agent_id: str, error_reason: str) -> None:
        ...
        try:
            await self.shared_state.update_agent_state(agent_id, 'error')
            await self.shared_state.log_system_event(
